...
//
float32_t	lms3_nr_delay[LMS_NR_DELAYBUF_SIZE_MAX+16];
//
float32_t errsig1[65];
float32_t errsig2[65];
float32_t errsig3[65];
float32_t result[65];
//
// LMS Filters for RX
...
//
// LMS Filters for RX
arm_lms_norm_instance_f32	lms3Norm_instance;
arm_lms_instance_f32	lms3_instance;
float32_t	lms3StateF32[192];
float32_t	lms3NormCoeff_f32[192];
//
//
...
void audio_driver_set_rx_audio_filter(void)
{
	...
	//
	lms3Norm_instance.numTaps = calc_taps;
	lms3Norm_instance.pCoeffs = lms3NormCoeff_f32;
	lms3Norm_instance.pState = lms3StateF32;
	//
...
//
	...
	arm_lms_norm_init_f32(&lms3Norm_instance, calc_taps, &lms3NormCoeff_f32[0], &lms3StateF32[0], (float32_t)mu_calc, 64);
	//
	//
...
static void audio_rx_processor(int16_t *src, int16_t *dst, int16_t size)
{
	...
	static ulong		lms3_inbuf = 0, lms3_outbuf = 0;
	...
	switch(ts.dmod_mode)	{
		...
		case DEMOD_AM:
			...
			for(i = 0; i < size/2; i+=4)	{										// square root of contents
				arm_sqrt_f32(ads.a_buffer[i], &ads.a_buffer[i]);				// Unroll this function for maximum MCU efficiency
				arm_sqrt_f32(ads.a_buffer[i+1], &ads.a_buffer[i+1]);
				arm_sqrt_f32(ads.a_buffer[i+2], &ads.a_buffer[i+2]);
				arm_sqrt_f32(ads.a_buffer[i+3], &ads.a_buffer[i+3]);
			}
	...
	if((ts.dsp_active & 1) && (ts.dsp_active & 2) && (!ads.af_dissabled) && (!ts.dsp_inhibit))	{	// Do DSP NR if enabled and if post-DSP NR enabled
		arm_copy_f32(ads.a_buffer, &lms3_nr_delay[lms3_inbuf], psize/2);	// put new data into the delay buffer
		//
		arm_lms_norm_f32(&lms3Norm_instance, ads.a_buffer, &lms3_nr_delay[lms3_outbuf], ads.a_buffer, errsig3, psize/2);
		//
		...
		//
		lms3_inbuf += psize/2;
		lms3_outbuf = lms3_inbuf + psize/2;
		if(lms3_inbuf >= ts.dsp_nr_delaybuf_len-1)
			lms3_inbuf -= ts.dsp_nr_delaybuf_len;
		if(lms3_outbuf >= ts.dsp_nr_delaybuf_len-1)
			lms3_outbuf -= ts.dsp_nr_delaybuf_len;
	}
	...
	if(ts.dmod_mode == DEMOD_AM)
		arm_scale_f32(ads.a_buffer,(float32_t)(ads.post_agc_gain * post_agc_gain_scaling * AM_SCALING), ads.a_buffer, psize/2);	// apply fixed amount of audio gain scaling to make the audio levels correct along with AGC
	else
...
static void audio_tx_processor(int16_t *src, int16_t *dst, int16_t size)
{
	...
	//
	if((ts.tune) && ((ts.dmod_mode != DEMOD_LSB) && (ts.dmod_mode != DEMOD_USB)))	// Tune mode - but NOT in USB/LSB mode
	{
	...
	else if((ts.dmod_mode == DEMOD_LSB) || (ts.dmod_mode == DEMOD_USB))		// Do this ONLY if in USB or LSB mode, of course!
	{
		...
		//
		// ------------------------
		// Do gain processing and output I and Q as stereo
		for(i = 0; i < size/2; i++)
		{
			// Equalise based on band  (Note:  the four statements below could be streamlined using a pair of "arm_scale()" calculations!)
			ads.i_buffer[i] *= (float)ts.tx_power_factor;
			ads.q_buffer[i] *= (float)ts.tx_power_factor;
	...
	// -----------------------------
	// AM handler - not implemented yet!!!
	//
	else if(ts.dmod_mode == DEMOD_AM)
	{
		for(i = 0; i < size; i++)
			*dst++ = *src++;

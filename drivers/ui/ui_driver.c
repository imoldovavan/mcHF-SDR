...
void ui_driver_toggle_tx(void)
{
	...
	if(ts.rit_value)		// Re-set frequency if RIT is non-zero
...
static void UiDrawSpectrumScopeFrequencyBarText(void)
{
	...
	if(!sd.magnify)	// magnify mode off
		grat = 6;	// graticules spaced 6 kHz
	else			// magnify mode on
	...
	freq_calc = (df.tune_new + 2000)/4000;	// get current frequency in kHz, rounding to the nearest kHz
	...
	sprintf(txt, "%u ", (unsigned)freq_calc-(3*(unsigned)grat));	// build string for next frequency
	c = &txt[strlen(txt)-3];  // point at 2nd character from the end
	...
	sprintf(txt, " %u ", (unsigned)freq_calc-(unsigned)grat);	// build string for next frequency
	c = &txt[strlen(txt)-3];  // point at 2nd character from the end
...
uchar UiDriverCheckBand(ulong freq, ushort update)
{
	uchar	band_scan;
	bool flag;
	static	uchar band_scan_old = 999;
...
*/
	...
	if(ts.txrx_mode == TRX_MODE_RX)		{
		tune_freq += (ts.rit_value*80);	// Add RIT on receive
	}
...
//	ts.dsp_inhibit = 1;				// disable DSP during tuning
	// Set frequency
	if(ui_si570_set_frequency(tune_freq,ts.freq_cal,df.temp_factor))
	{
		if(ui_si570_set_frequency(tune_freq,ts.freq_cal,df.temp_factor))	// Try again if it didn't work the first time
...
//	ts.dsp_inhibit = dsp_temp;		// restore status of DSP muting
	...
	//
	UiDriverCheckFilter(tune_freq/4);	// check the filter status with the new frequency update
	UiDriverCheckBand(tune_freq, 1);		// check which band in which we are currently tuning and update the display
	//
...
void UiDriverUpdateFrequencyFast(void)
{
	...
	//
	// Do "Icom" style frequency offset of the LO if in "CW OFFSET" mode
	//
	if(ts.dmod_mode == DEMOD_CW)	{
		if(ts.cw_offset_mode == CW_OFFSET_USB_SHIFT)
			dial_freq -= ts.sidetone_freq;
		else if(ts.cw_offset_mode == CW_OFFSET_LSB_SHIFT)
			dial_freq += ts.sidetone_freq;
		else if(ts.cw_offset_mode == CW_OFFSET_AUTO_SHIFT)	{
			if(ts.cw_lsb)
				dial_freq += ts.sidetone_freq;
			else
				dial_freq -= ts.sidetone_freq;
...
*/
	...
	// Set frequency
	ui_si570_set_frequency(tune_freq,ts.freq_cal,df.temp_factor);
...
//	df.update_skip = 0;
	...
	if(ts.frequency_lock)
		return;						// frequency adjust is locked
...
//	sd.skip_process = 0;
	...
	switch(sd.state)
	{
		...
		case 1:
		{
			...
			// 8 bits of shift turns out to be a bit too much, so we do just 6...
			//
...
//* Object              : Calculate TX audio compression settings
//* Input Parameters    :
//* Output Parameters   :
//* Functions called    :
//*----------------------------------------------------------------------------
void UiCalcTxCompLevel(void)
{
	...
	switch(ts.tx_comp_level)	{
		case 0:		// minimum compression
			ts.alc_tx_postfilt_gain = 1;
			ts.alc_decay = 15;
			break;
		case 1:

...
uchar UiLcdHy28_InitA(void)
{
	...
	if(DeviceCode == 0x9320)
	{
		...
		UiLcdHy28_Delay(200000);            // Dis-charge capacitor power voltage (200ms)
...
void UiLcdHy28_ShowStartUpScreen(ulong hold_time)
{
	...
	UiLcdHy28_PrintText(70,100,tx,White,Black,0);

	// Show second line
	sprintf(tx,"%s",AUTHOR_STRING);
	UiLcdHy28_PrintText(60,120,tx,White,Black,0);

	// Show third line
	sprintf(tx,"v %d.%d.%d.%d",TRX4M_VER_MAJOR,TRX4M_VER_MINOR,TRX4M_VER_RELEASE,TRX4M_VER_BUILD);
	UiLcdHy28_PrintText(110,140,tx,White,Black,0);

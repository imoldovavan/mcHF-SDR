...
//* Functions called    :
//*----------------------------------------------------------------------------
static void UiLcdHy28_SendByteSpi(uint8_t byte)
{
...
uchar UiLcdHy28_InitA(void)
{
   ...
   if(DeviceCode == 0x5408)
   {
      ...
      UiLcdHy28_WriteReg(0x0080,0x0000);   // Display Position 每 Partial Display 1
      UiLcdHy28_WriteReg(0x0081,0x0000);   // RAM Address Start 每 Partial Display 1
      UiLcdHy28_WriteReg(0x0082,0x0000);   // RAM address End - Partial Display 1
      UiLcdHy28_WriteReg(0x0083,0x0000);   // Display Position 每 Partial Display 2
      UiLcdHy28_WriteReg(0x0084,0x0000);   // RAM Address Start 每 Partial Display 2
      UiLcdHy28_WriteReg(0x0085,0x0000);   // RAM address End 每 Partail Display2
...
void UiLcdHy28_ShowStartUpScreen(ulong hold_time)
{
 ...
 //  uint16_t    t, s, u, v;
   ...
   UiLcdHy28_PrintText(60,80,tx,White,Black,0);

   // Show third line
   sprintf(tx,"v %d.%d.%d.%d",TRX4M_VER_MAJOR,TRX4M_VER_MINOR,TRX4M_VER_RELEASE,TRX4M_VER_BUILD);
   UiLcdHy28_PrintText(110,100,tx,White,Black,0);
   ...
   if(!(i & 0xff))	{
	   ...
	   UiLcdHy28_PrintText(80,140,tx,Grey3,Black,0);
   }
   ...
   if(ts.ee_init_stat != FLASH_COMPLETE)	{	// Show error code if problem with EEPROM init
	   ...
	   UiLcdHy28_PrintText(60,155,tx,Grey2,Black,0);
   }
